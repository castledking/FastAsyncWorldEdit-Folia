...
import static com.sk89q.worldedit.bukkit.adapter.impl.fawe.v1_21_8.PaperweightPlatformAdapter.createInput;
...
public class PaperweightFaweWorldNativeAccess implements WorldNativeAccess<LevelChunk,
    ...
    public PaperweightFaweWorldNativeAccess(PaperweightFaweAdapter paperweightFaweAdapter, WeakReference<Level> level) {
        ...
        this.level = level;
        // In Folia, we can't reliably get the server tick, so we'll use a timestamp-based approach
        // This is a simple counter that increments every millisecond
        // We divide by 50 to approximate ticks (20 ticks per second = 1 tick every 50ms)
        this.lastTick = new AtomicInteger((int)(System.currentTimeMillis() / 50));
    }
    ...
    private synchronized void flushAsync(final boolean sendChunks) {
        ...
        RunnableVal<Object> runnableVal = new RunnableVal<>() {
            ...
            public void run(Object value) {
                ...
                for (IntPair chunkPos : toSend) {
                    org.bukkit.Chunk chunk = getLevel().getWorld().getChunkAt(chunkPos.x(), chunkPos.z());
    ...
    public synchronized void flush() {
        RunnableVal<Object> runnableVal = new RunnableVal<>() {
            ...
            public void run(Object value) {
                ...
                for (IntPair chunkPos : cachedChunksToSend) {
                    org.bukkit.Chunk chunk = getLevel().getWorld().getChunkAt(chunkPos.x(), chunkPos.z());
                    PaperweightPlatformAdapter.sendChunk(chunk);
                }
            }
